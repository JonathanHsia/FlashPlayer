...
public class FileUtils {
	...
	/** 获取指定目录下的指定后缀的文件 */
	public static List<File> getFileListBySuffix(String path, String suffix, List<File> data) {
		File f = new File(path);
		return getFileListBySuffix(f, suffix, data);
	}
	/**获取指定目录下的指定后缀的文件重载方法*/
	public static List<File> getFileListBySuffix(File filePath, String suffix,
			List<File> data) {
		if (filePath.isDirectory()) {
			File[] fs = filePath.listFiles();
			for (int i = 0; i < fs.length; i++) {
				data = getFileListBySuffix(fs[i].getPath(), suffix, data);
			}
		} else if (filePath.getName().endsWith(suffix)) {
			data.add(filePath);
		}
		return data;
	}
	/**删除给定集合目录中的所有文件*/
	public static void deleteFileByFileList(List<File> data){
		File tempFile = null;
		for (int i = 0; i < data.size(); i++) {
			tempFile = data.get(i);
			if(tempFile.exists()&&tempFile.length()>0){//给定文件存在
				tempFile.delete();
			}
		}
	}
